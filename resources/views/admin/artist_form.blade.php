...
<head>
    ...
    <title>Tentacit Records | {{ $artist ? 'Edit' : 'Add' }} Artist</title>
...
<body>
    ...
    <div class="main-content">
        <div class="header">
            <h1>{{ $artist ? 'Edit' : 'Add' }} <span>Artist</span></h1>
        </div>
        ...
        <div class="form-card">
            <form action="{{ route('admin.artist.save', $artist->id ?? '') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="row g-3">
                    <div class="col-md-6">
                        <label class="form-label">Artist Name</label>
                        <input type="text" name="artistname" class="form-control" value="{{ $artist->artistname ?? '' }}" required>
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">Real Name</label>
                        <input type="text" name="realname" class="form-control" value="{{ $artist->realname ?? '' }}">
                    </div>
                    <div class="col-md-6">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" name="nationality" class="form-control" value="{{ $artist->nationality ?? '' }}">
                    </div>
                    <div class="col-md-12">
                        <label class="form-label">Info</label>
                        <textarea name="info" class="form-control" rows="4">{{ $artist->info ?? '' }}</textarea>
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">Profile Image</label>
                        <input type="file" name="image" class="form-control" accept="image/*">
                        @if($artist && $artist->image)
                            <div class="current-file">Current: {{ $artist->image }}</div>
                            <img src="{{ asset('artist-profile-images/' . $artist->image) }}" class="preview-img">
                        @endif
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">Background Image 1</label>
                        <input type="file" name="background_img" class="form-control" accept="image/*">
                        @if($artist && $artist->background_img)
                            <div class="current-file">Current: {{ $artist->background_img }}</div>
                            <img src="{{ asset('artist-profile-images/' . $artist->background_img) }}" class="preview-img">
                        @endif
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">Background Image 2</label>
                        <input type="file" name="secondbackground_img" class="form-control" accept="image/*">
                        @if($artist && $artist->secondbackground_img)
                            <div class="current-file">Current: {{ $artist->secondbackground_img }}</div>
                            <img src="{{ asset('artist-profile-images/' . $artist->secondbackground_img) }}" class="preview-img">
                        @endif
                    </div>
                </div>
                <div class="mt-4 d-flex gap-3">
                    <button type="submit" class="btn-save"><i class="fas fa-save"></i> {{ $artist ? 'Update' : 'Save' }} Artist</button>
                    <a href="{{ route('admin.artists') }}" class="btn-cancel"><i class="fas fa-times"></i> Cancel</a>
                </div>
            </form>
        </div>
    </div>
